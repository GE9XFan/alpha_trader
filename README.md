...
| Module | Lines | Primary Classes | Responsibilities | Redis Touchpoints |
| --- | --- | --- | --- | --- |
| `execution_manager.py` | 870 | `ExecutionManager` | Routes IBKR orders, enforces max positions, monitors fills, and posts execution telemetry.【F:src/execution_manager.py†L33-L533】 | `signals:pending:{symbol}`, `orders:*`, `positions:open:*`, `health:execution:heartbeat`, `alerts:critical`【F:src/execution_manager.py†L1-L31】 |
| `position_manager.py` | 780 | `PositionManager` | Maintains P&L, trailing stops, scaling logic, and closed-position archives.【F:src/position_manager.py†L30-L459】 | `positions:open:*`, `positions:closed:*`, `positions:pnl:*`, `risk:daily_pnl`【F:src/position_manager.py†L1-L22】 |
| `risk_manager.py` | 803 | `RiskManager` | Evaluates daily loss gates, VaR thresholds, and correlation halts; broadcasts circuit breaker states.【F:src/risk_manager.py†L31-L401】 | `risk:*`, `circuit_breakers:*`, `alerts:critical`, `system:halt`【F:src/risk_manager.py†L1-L22】 |
| `emergency_manager.py` | 698 | `EmergencyManager`, `CircuitBreakers` | Handles mass cancels, emergency halts, and breaker resets, coordinating with risk telemetry.【F:src/emergency_manager.py†L35-L424】 | `risk:emergency:*`, `orders:emergency:*`, `alerts:emergency`, `metrics:emergency:*`, `circuit_breakers:*`【F:src/emergency_manager.py†L1-L32】 |
