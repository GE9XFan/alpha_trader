...
| `signal_distributor.py` | 333 | `SignalDistributor`, `SignalValidator` | Manages queue tiering and downstream distribution checks for different subscriber classes.【F:src/signal_distributor.py†L25-L274】 | `distribution:*`, `signals:pending`, `metrics:signals:*`【F:src/signal_distributor.py†L1-L13】 |
| `signal_performance.py` | 122 | `PerformanceTracker` | Tracks win/loss metrics, latency, and strategy scorecards for emitted signals.【F:src/signal_performance.py†L25-L219】 | `metrics:signals:*`, `signals:audit:*`【F:src/signal_performance.py†L2-L12】 |
