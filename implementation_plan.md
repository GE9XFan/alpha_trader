...
- **Phase 5 – Backfills, monitoring, and regression coverage** (In Progress): Historical backfill utilities now replay dealer-flow, flow-cluster, and VIX1D series through dedicated jobs, while monitoring dashboards and expanded regression suites remain open items.
...
- **Historical backfill jobs** – Implemented via the `backfill` package and CLI, enabling dealer-flow, flow clustering, and VIX1D replays with resumable checkpoints and regression coverage seeded in `tests/test_backfill_jobs.py`. Next steps include wiring automated backfill triggers into operations playbooks and validating performance against multi-day windows.【F:src/backfill/cli.py†L14-L162】【F:tests/test_backfill_jobs.py†L14-L162】
- **Monitoring & replay analytics** – Build dashboards/notebooks around the new metrics (success rates, z-score drift, cluster stability) and surface backfill progress plus anomaly alerts once the monitoring collectors are in place.
- **Expanded regression coverage** – Extend automated suites to stress DealerFlowCalculator edge cases, signal-scoring paths that combine the new features, and the hardened execution/distribution workflows (fills, partial scale-outs, trailing-stop rebuilds).
...
- **Dashboard services** – Flesh out the FastAPI routes/WebSocket broadcasting, log aggregation, alert management, and performance chart builders so operators (and community members, if desired) have a real-time lens on health and risk.【F:src/dashboard_server.py†L25-L220】【F:src/dashboard_routes.py†L25-L184】【F:src/dashboard_websocket.py†L1-L70】
...
## Open Work
- [Phase 5 – Outstanding Follow-Ups](#phase-5--outstanding-follow-ups)
