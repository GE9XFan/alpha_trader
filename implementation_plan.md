...
- **Intraday P&L reconciliation (2025-09-16)**: Added `reconcile_daily_pnl.py` to fetch IBKR fills, aggregate realized P&L per account/contract, and rewrite Redis metrics (`risk:daily_pnl`, `positions:pnl:realized:total`, `positions:closed:<date>:<account>:<conId>`). ExecutionManager now records fill-level `realized_events`, honors IBKR `commissionReport.realizedPNL`, and persists realized/commission totals so Redis mirrors TWS immediately after each exit.【F:reconcile_daily_pnl.py†L1-L259】【F:src/execution_manager.py†L320-L1950】
- **State inspection UX (2025-09-16)**: `redis_data_viewer.py` now surfaces closed positions with IBKR contract identifiers, Eastern entry/exit times, net P&L, commissions, and fill counts while filtering reconciled placeholders; the risk panel decodes drawdown history when available and flags missing metrics for operator awareness.【F:redis_data_viewer.py†L140-L370】
- **Position hygiene & P&L integrity**: `PositionManager` now ignores zero-quantity IBKR snapshots, prunes local caches when Redis keys disappear, and limits P&L aggregation to open quantities so ExecutionManager-owned realized/daily metrics remain intact instead of being overwritten every heartbeat.【F:src/position_manager.py†L165-L427】
- **Strategy-aware execution sizing**: `ExecutionManager` normalizes strategy casing before sizing orders, routing market/limit decisions, and deriving bracket defaults, restoring the intended 0DTE/1DTE contract caps for lowercase signals emitted by the strategy stack.【F:src/execution_manager.py†L808-L1452】
