...
| `src/pattern_analyzer.py` (`PatternAnalyzer`) | Assess flow toxicity, order book imbalance, sweep detection, hidden/iceberg inference | `analyze_flow_toxicity`, `calculate_order_book_imbalance`, `detect_sweeps`, `detect_hidden_orders` | `analytics:{symbol}:toxicity`, `analytics:{symbol}:obi`, `analytics:{symbol}:metric:*`, `market:{symbol}:book`, `market:{symbol}:trades`, `analytics:{symbol}:vpin` |
...
- Downstream modules consume the adjusted score (`toxicity_adjusted`) and component ratios (`aggressor_ratio`, `large_trade_ratio`, `institutional_score`) directly via the feature reader.
